...
class progressthread(QtCore.QThread):
    ...
    resultxt = QtCore.pyqtSignal(str)

    def run(self):
        for everynum in range(101):
            self.result.emit(everynum)
            self.resultxt.emit(str(everynum))
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.thread.result.connect(self.progressBar.setValue)
        self.thread.resultxt.connect(self.lineEdit.setText)
